...
import { resolveStoryScope,type StoryReadScope } from './scope.js';
import { foreshadowProjection } from './foreshadow.js';
import { projectReadIndex,type ProjectReadIndex } from './read-model.js';
...
export const memoryList=(store:Store,pid:string,index?:ProjectReadIndex):MemoryRecord[]=>index?index.memoryRows<MemoryRecord>():store.db.prepare('SELECT data FROM memories WHERE projectId=? ORDER BY rowid').all(pid).map(r=>JSON.parse(r.data as string));
...
export function sourceFor(domain:Domain,c:StoryObject,v?:ChapterVersion,index:ProjectReadIndex=projectReadIndex(domain,c.projectId)):MemorySource {return index.sourceFor(c,v);}
function sourceMatches(domain:Domain,c:StoryObject,source:MemorySource,index:ProjectReadIndex){const current=sourceFor(domain,c,undefined,index);if(source.structureHash===undefined)delete current.structureHash;return stable(current)===stable(source);}
export function validMemory(domain:Domain,m:MemoryRecord,index:ProjectReadIndex=projectReadIndex(domain,m.projectId)){return m.status==='valid'&&m.sources.every(s=>{const c=index.byId.get(s.chapterId);return c&&c.kind==='chapter'&&c.status==='accepted'&&!c.fields.needsReview&&sourceMatches(domain,c,s,index);});}
...
export function installMemory(domain:Domain,pid:string,source:MemorySource,raw:unknown,actor:string,taskId?:string,checkpointEvery=5){
 const index=projectReadIndex(domain,pid),c=index.object(source.chapterId);requireThat(c.status==='accepted'&&!c.fields.needsReview&&sourceMatches(domain,c,source,index),'STALE','记忆来源正文、顺序或分支已改变；旧结果不能提交');
 ...
 if(old?.locked){requireThat(validMemory(domain,old,index),'MEMORY_LOCKED','锁定记忆的正文来源已失效；需要作者解锁后重建');return old;}
 if(old&&validMemory(domain,old,index)&&stable(old.content)===stable(content))return old;
...
export function sealCheckpoints(domain:Domain,pid:string,every=5){
 const index=projectReadIndex(domain,pid),chapters=index.chapters,memories=memoryList(domain.store,pid,index),byChapter=new Map(memories.filter(m=>m.kind==='chapter'&&validMemory(domain,m,index)).map(m=>[m.sources[0].chapterId,m]));for(let start=0;start<chapters.length;start+=every){const group=chapters.slice(start,start+every);if(group.length<every)continue;const selected=group.map(c=>byChapter.get(c.id));if(selected.some(m=>!m))continue;
 const sources=selected.flatMap(m=>m!.sources);if(new Set(sources.map(s=>s.branch)).size!==1)continue;const sourceHash=hash(stable({sources,memories:selected.map(m=>[m!.id,m!.revision])}));if(memories.some(m=>m.kind==='checkpoint'&&m.sourceHash===sourceHash&&validMemory(domain,m,index)))continue;
...
export function memoryStatus(domain:Domain,pid:string,read?:StoryReadScope){const index=read?.index??projectReadIndex(domain,pid),chapters=index.chapters.filter(c=>c.status==='accepted'&&c.body.trim()&&!c.fields.referenceOnly);const memories=memoryList(domain.store,pid,index).map(m=>({...m,status:validMemory(domain,m,index)?'valid' as const:'stale' as const}));const covered=new Set(memories.filter(m=>m.kind==='chapter'&&m.status==='valid').map(m=>m.sources[0].chapterId));const gaps=chapters.filter(c=>!covered.has(c.id)).map(c=>({...sourceFor(domain,c,undefined,index),title:c.title,chars:c.body.length,needsReview:!!c.fields.needsReview}));const waterline=chapters.findIndex(c=>!covered.has(c.id));return {total:chapters.length,covered:covered.size,gaps,waterline:waterline<0?chapters.length:waterline,memories};}
...
export function recallMemory(domain:Domain,pid:string,options:{goal:string;asOf:number;branch?:string;entityIds?:string[];viewpointId?:string;audience?:string;limit?:number;scope?:StoryReadScope}){
 ...
 const chapters=candidates.filter(visible),allowed=new Set(chapters.map(c=>c.id)),index=options.scope?.index??projectReadIndex(domain,pid);const records=memoryList(domain.store,pid,index).filter(m=>m.kind==='chapter'&&validMemory(domain,m,index)&&m.sources.every(s=>allowed.has(s.chapterId))),byChapter=new Map(records.map(m=>[m.sources[0].chapterId,m]));
 ...
 for(const c of chapters){const memory=byChapter.get(c.id);const pov=options.audience==='character'?options.viewpointId:undefined;const scenes=memory?.content.scenes.filter(s=>!pov||s.knownByIds.includes(pov))??[];const fullAllowed=!pov||c.fields.viewpointId===pov||c.fields.public===true;
