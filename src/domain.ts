...
export class Domain {
  ...
  restoreBackup(input:unknown){
    ...
    return this.store.transaction(()=>{const project={...data.project,...p,id:ids.get(data.project.id)!,title:p.title+' · 恢复副本',archived:false};this.store.put('projects',project);
      ...
      for(const m of data.memories??[]){requireThat(m.projectId===data.project.id,'BACKUP','记忆归属无效',422);const value=remap(m);if(value.kind==='chapter')value.sourceHash=hash(stable(value.sources));else {const children=value.sources.map((source:any)=>(data.memories??[]).map(remap).find((x:any)=>x.kind==='chapter'&&x.status==='valid'&&stable(x.sources[0])===stable(source)));value.sourceHash=hash(stable({sources:value.sources,memories:children.filter(Boolean).map((x:any)=>[x.id,x.revision])}));}this.store.db.prepare('INSERT INTO memories VALUES(?,?,?)').run(value.id,project.id,JSON.stringify(value));}
