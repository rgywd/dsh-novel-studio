...
import { requestExport,requestRestore,requestIdsIn,writerRequestForArtifact } from './requests.js';
...
import { coverGet,decodeCover } from './shelf.js';
...
export class Domain {
  ...
  putArtifact(task:CreativeTask,type:Artifact['type'],data:any,chapter?:StoryObject):Artifact {
    const request=typeof data.content==='string'?writerRequestForArtifact(this.store,task.projectId,task.id,data.content):undefined;
  ...
  acceptArtifact(projectId:string,artifactId:string,actor:'author'|'auto',partialText?:string){
    return this.store.transaction(()=>{
      ...
      if(a.type==='memory'){
        ...
        const requestIds=[...new Set([...requestIdsIn(a.data),...t.steps.flatMap(step=>step.requestIds??[])])];
        const version:ChapterVersion={id:id('version'),projectId,chapterId:c.id,content:a.data.content,chapterRevision:c.revision+1,createdAt:now(),actor:actor==='auto'?`auto:${t.id}:qualified-v1`:'author',accepted:true,summary:review.summary,commitKey:a.id,requestIds};
        ...
        this.invalidateDerived(projectId,c.id,true);const content=c.body.slice(0,range.start)+replacement+c.body.slice(range.end);const v:ChapterVersion={id:id('version'),projectId,chapterId:c.id,content,chapterRevision:c.revision+1,createdAt:now(),actor:'author-selection',accepted:true,summary:'',commitKey:a.id,requestIds:[...new Set([...requestIdsIn(a.data),...t.steps.flatMap(step=>step.requestIds??[])])]};this.store.put('versions',v);this.store.put('objects',{...c,body:content,status:'accepted',revision:c.revision+1,fields:{...c.fields,currentVersion:v.id,summaryVersion:'',extractionPending:true},updatedAt:now()});
  ...
  backup(projectId:string){const tasks=this.store.list('tasks',projectId);const effective=resolveConfig(this.store,projectId);const configIds=new Set([...effective.versions,...tasks.flatMap(t=>t.configSnapshot?.versions??[])].map(v=>v.id));const cover=coverGet(this,projectId);const payload={schema:2,project:this.project(projectId),objects:this.store.objects(projectId),versions:this.store.list('versions',projectId),tasks,artifacts:this.store.list('artifacts',projectId),events:this.store.db.prepare('SELECT * FROM events WHERE projectId=?').all(projectId),changesets:this.store.db.prepare('SELECT * FROM changesets WHERE projectId=?').all(projectId),imports:this.store.db.prepare('SELECT * FROM imports WHERE projectId=?').all(projectId),configurations:configAncestry(this.store,configIds),effectiveConfiguration:effective,memories:this.store.db.prepare('SELECT data FROM memories WHERE projectId=?').all(projectId).map(r=>JSON.parse(r.data as string)),requests:requestExport(this.store,projectId),...(cover?{cover:{mime:cover.mime,base64:Buffer.from(cover.data).toString('base64')}}:{})};return {checksum:hash(payload),payload};}
  restoreBackup(input:unknown){
    const envelope=z.object({checksum:z.string(),payload:z.object({schema:z.union([z.literal(1),z.literal(2)]),project:z.any(),objects:z.array(z.any()),versions:z.array(z.any()),tasks:z.array(z.any()),artifacts:z.array(z.any()),events:z.array(z.any()),changesets:z.array(z.any()),imports:z.array(z.any()),configurations:z.array(z.any()).optional(),effectiveConfiguration:z.any().optional(),memories:z.array(z.any()).optional(),requests:z.array(z.any()).optional(),cover:z.object({mime:z.string(),base64:z.string()}).optional()})}).parse(input);
    ...
    return this.store.transaction(()=>{const project={...data.project,...p,id:ids.get(data.project.id)!,title:p.title+' · 恢复副本',archived:false};this.store.put('projects',project);
      ...
      for(const r of data.requests??[]){requireThat(r.projectId===data.project.id&&ids.has(r.taskId),'BACKUP','请求归属无效',422);const value=remap(r);value.historical=true;requestRestore(this.store,value);}
      if(data.cover)this.store.db.prepare('INSERT INTO project_covers VALUES(?,?,?,?,?)').run(project.id,1,data.cover.mime,decodeCover(data.cover.mime,data.cover.base64),now());
