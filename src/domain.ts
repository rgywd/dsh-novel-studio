...
export class Domain {
  ...
  updateObject(projectId:string,objectId:string,revision:number,input:unknown){
    ...
    return this.store.transaction(()=>{
      ...
      if(old.kind==='chapter'&&(old.order!==data.order||old.parentId!==data.parentId||old.fields.branch!==data.fields.branch)){this.invalidateDerived(projectId,this.chapters(projectId)[0].id,true);const invalidated=this.object(projectId,old.id);data.fields={...data.fields,summaryVersion:'',extractionPending:true,needsReview:invalidated.fields.needsReview};}
  ...
  createTask(projectId:string,input:unknown):CreativeTask {
    ...
    this.store.transaction(()=>{this.store.put('tasks',t);this.store.event(projectId,t.id,'task.queued','任务约定已保存，可开始执行',{scope:t.contract.scope,budget:t.budget,configuration:t.configSnapshot!.hash});});return t;
  }
  ...
  commitEvidence(chapter:StoryObject,version:ChapterVersion,review:Review,task:CreativeTask,actor:string){
    ...
    for(const claim of review.claims){requireThat(version.content.includes(claim.quote),'EVIDENCE','事实证据未出现在正文');this.object(chapter.projectId,claim.entityId);this.addObject(chapter.projectId,{kind:'fact',title:`${claim.property}：${claim.value}`,body:claim.quote,status:claim.inference||!['objective','knowledge'].includes(claim.modality)?'candidate':'accepted',fields:{entityId:claim.entityId,property:claim.property,value:claim.value,branch:String(chapter.fields.branch??'main')},source:{...source(claim.quote),time:claim.time,supersedes:claim.supersedes,inference:claim.inference,modality:claim.modality}});}
  ...
  restoreBackup(input:unknown){
    ...
    return this.store.transaction(()=>{const project={...data.project,...p,id:ids.get(data.project.id)!,title:p.title+' · 恢复副本',archived:false};this.store.put('projects',project);
      ...
      if(data.effectiveConfiguration){const configuration=configPatchSchema.parse(remap(data.effectiveConfiguration.config));const version={id:id('config'),name:'恢复时的有效配置',createdAt:now(),converter:'restore/2',format:'native',source:'备份恢复（独立于全局默认）',config:configuration,report:[],unknown:{},applicable:true};saveConfig(this.store,version);this.store.db.prepare('INSERT INTO config_bindings VALUES(?,?)').run('project:'+project.id,version.id);}
      for(const m of data.memories??[]){requireThat(m.projectId===data.project.id,'BACKUP','记忆归属无效',422);const value=remap(m);this.store.db.prepare('INSERT INTO memories VALUES(?,?,?)').run(value.id,project.id,JSON.stringify(value));}
