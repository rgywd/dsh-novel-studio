...
export class Domain {
  ...
  controlTask(projectId:string,taskId:string,action:'pause'|'cancel'|'resume'|'redelegate'){
  ...
  commitEvidence(chapter:StoryObject,version:ChapterVersion,review:Review,task:CreativeTask,actor:string){
    ...
    for(const claim of review.claims){requireThat(version.content.includes(claim.quote),'EVIDENCE','事实证据未出现在正文');this.object(chapter.projectId,claim.entityId);this.addObject(chapter.projectId,{kind:'fact',title:`${claim.property}：${claim.value}`,body:claim.quote,status:claim.inference||!['objective','knowledge'].includes(claim.modality)?'candidate':'accepted',fields:{entityId:claim.entityId,property:claim.property,value:claim.value},source:{...source(claim.quote),time:claim.time,inference:claim.inference,modality:claim.modality}});}
    for(const e of review.events){requireThat(version.content.includes(e.quote),'EVIDENCE','事件证据未出现在正文');this.addObject(chapter.projectId,{kind:'event',title:e.title,body:e.quote,status:'accepted',fields:{entityIds:e.entityIds,storyTime:e.time,chapterOrder:ordinal},source:{...source(e.quote),time:e.time}});}
  ...
  importText(projectId:string,revision:number,name:string,raw:string,parts?:{title:string;body:string}[]){
    ...
    return this.store.transaction(()=>{this.guard(projectId,revision);this.invalidateTasks(projectId,'导入了新正文');this.store.db.prepare('INSERT INTO imports VALUES(?,?,?,?,?)').run(id('import'),projectId,name,raw,now());const book=this.store.objects(projectId,'book')[0];const volume=this.addObject(projectId,{kind:'volume',title:name,parentId:book.id,status:'planned',order:this.store.objects(projectId,'volume').length});
