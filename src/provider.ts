...
export interface ModelResult {text:string;outputTokens?:number;estimated?:boolean;}
...
export class DshProvider implements ModelProvider {
  constructor(private llm:{stream:(options:GenerateOptions)=>AsyncIterable<StreamChunk>},private selection:()=>{provider:string;model:string;reasoningEffort?:any}){}
  ...
  async generate(r:ModelRequest):Promise<ModelResult>{
    ...
    for await(const chunk of this.llm.stream(options)){
      ...
      if(chunk.type==='finish'){
        ...
        if(chunk.reason.kind!=='stop')throw new DomainError('INCOMPLETE_OUTPUT','模型未完整结束正文；已保留输出，不能自动接受',502);
      }
    }
