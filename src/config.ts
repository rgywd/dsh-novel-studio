...
export function safeRaw(raw:string){requireThat(raw.length<=1_000_000,'CONFIG_SIZE','配置文件限 100 万字符',413);let value:any;try{value=JSON.parse(raw);}catch{throw new DomainError('CONFIG_JSON','配置不是合法 JSON',422);}
...
export function saveConfig(store:Store,v:ConfigVersion){v={...v,config:configPatchSchema.parse(v.config)};if(v.parentId){const parent=configVersion(store,v.parentId);v.report=[...new Map([...parent.report,...v.report].map(r=>[stable(r),r])).values()];}store.db.prepare('INSERT INTO config_versions(id,data) VALUES(?,?)').run(v.id,JSON.stringify(v));return v;}
export function configAncestry(store:Store,ids:Iterable<string>){const all=new Map(listConfigs(store).map(v=>[v.id,v])),seen=new Set<string>(),result:ConfigVersion[]=[];const visit=(key:string)=>{if(seen.has(key))return;seen.add(key);const v=all.get(key);if(!v)return;if(v.parentId)visit(v.parentId);result.push(v);};for(const key of ids)visit(key);return result;}
