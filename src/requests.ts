...
export interface RequestRecord {id:string;projectId:string;taskId:string;stepKey:string;attempt:number;createdAt:string;status:'PREPARED'|'SENT'|'COMPLETED'|'FAILED'|'STALE';compiled:CompiledPrompt;observed?:unknown;observedAt?:string;observedChars?:number;observedEstimatedTokens?:number;previousId?:string;diff?:ReturnType<typeof requestDiff>;localPrefixReuse?:boolean;usage?:UsageRecord;rawResponse?:string;candidate?:string;syntaxRepair?:{removedTrailingClosers:number};transformations?:Transformation[];error?:{code:string;message:string};}
...
export function observeRequest(store:Store,record:RequestRecord,observed:unknown){
  ...
  record.observed=observed;record.observedChars=JSON.stringify(observed).length;record.observedEstimatedTokens=Math.ceil(record.observedChars/2);record.observedAt=now();if(record.status==='PREPARED')record.status='SENT';record.previousId=previous?.id;record.diff=previous?requestDiff(previous.observed,observed):undefined;record.localPrefixReuse=previous?previous.compiled.stableHash===record.compiled.stableHash:undefined;return requestPut(store,record);
}
