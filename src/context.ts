...
export function buildContext(domain:Domain,projectId:string,options:{chapterId?:string;goal?:string;maxChars?:number;entityIds?:string[]}={}):ContextPack {
  ...
  const all=domain.store.objects(projectId);const chapters=domain.chapters(projectId);const current=options.chapterId?domain.object(projectId,options.chapterId):undefined;
  ...
  const relevant=new Set(options.entityIds??[]);const query=[options.goal,current?.title,current?.fields.goal,current?.fields.participants,current?.fields.events].filter(Boolean).join(' ');
  ...
  const canon=all.filter(o=>o.kind==='fact'&&o.status==='accepted'&&!o.source?.inference&&['objective','knowledge'].includes(o.source?.modality??'objective')&&latest(o)&&(!o.source?.chapterId||(ordinal.get(o.source.chapterId)??Infinity)<=index)&&(!o.source?.fromChapter||o.source.fromChapter<=index+1)&&(!o.source?.toChapter||o.source.toChapter>=index+1));
  ...
  for(const o of all.filter(o=>o.locked&&['character','world','fact','book','volume'].includes(o.kind))){add(o.id,String(o.revision),'locked','锁定约束完整保留',100,true,serialize(o));}
  for(const o of all.filter(o=>o.kind==='book'||o.id===current?.parentId)){if(!o.locked)add(o.id,String(o.revision),'plan','上层规划；未来内容不是既成事实',90,false,serialize(o));}
  if(current)add(current.id,String(current.revision),'chapter-plan','当前章目标、参与者、禁区、字数、揭示和结尾',98,true,JSON.stringify({id:current.id,title:current.title,fields:current.fields,tags:current.tags,locked:current.locked}));
  ...
  for(const o of all.filter(o=>['character','world','relationship'].includes(o.kind)&&o.status==='accepted'&&!o.locked)){
