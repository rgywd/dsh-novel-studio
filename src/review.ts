...
export function validateReview(raw:unknown,body:string,objects:StoryObject[],canon:StoryObject[],targetWords?:number):Review {
  ...
  for(const r of parsed.relationships??[]){requireThat(r.fromId!==r.toId&&[r.fromId,r.toId,...r.knownByIds].every(x=>byId.get(x)?.kind==='character'),'INVALID_ENTITY','关系变化两端和知情者必须为已有角色',422);requireThat(body.includes(r.quote),'INVALID_EVIDENCE','关系变化必须引用本章原文',422);}
  if(parsed.memory)for(const key of ['scenes','obligations'] as const)for(const [n,scene] of parsed.memory[key].entries()){
    requireThat(body.includes(scene.quote),'INVALID_EVIDENCE',`memory.${key}[${n}].quote 不是 supplied draft 的连续原文：${scene.quote.slice(0,90)}。历史背景/原作义务未在本章发生时，不重复登记为本章记忆；已有来源仍保留。`,422);
    requireThat(scene.entityIds.every(x=>byId.has(x)),'INVALID_ENTITY',`memory.${key}[${n}].entityIds 含未知ID，只能使用本书对象ID，不能使用sourceAssetId`,422);
    if('knownByIds' in scene)requireThat(scene.knownByIds.every(x=>byId.get(x)?.kind==='character'),'INVALID_ENTITY',`memory.scenes[${n}].knownByIds 只能是本书角色ID`,422);
  }
