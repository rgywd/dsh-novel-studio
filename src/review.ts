...
export function validateReview(raw:unknown,body:string,objects:StoryObject[],canon:StoryObject[],targetWords?:number):Review {
  ...
  if(parsed.memory)for(const scene of [...parsed.memory.scenes,...parsed.memory.obligations])requireThat(body.includes(scene.quote)&&scene.entityIds.every(x=>byId.has(x)),'INVALID_EVIDENCE','记忆证据或实体无效',422);
