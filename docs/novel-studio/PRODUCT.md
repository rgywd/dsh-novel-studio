...
## 第三阶段增量约定：全局书架与项目会话壳（2026-09-16）

未选作品时提供同一个全局工作空间：作品书架、原作资料和创作资产。已有作品使用紧凑网格/列表、搜索、最近章节、待审成果数、可替换封面和归档恢复；首次空书架保留灵感、已有正文及原作派生路径。书架数据只读项目元数据与增量投影，不为浏览封面加载正文或创建导演任务。

原作资料继续使用原有只读源、分析、继承清单与激活流程；创作资产复用 `config_versions` / `config_bindings`，不另建配置账本。保存版本与应用全局默认是两个动作，全局规则不得绑定某个项目的可变角色，运行中的任务仍使用固定快照。进入作品后继续使用原 Writer / Director，共同项目和章节身份不变；URL 可恢复当前项目，浏览器本地仅记忆工作模式、页面、章节与任务定位，脏正文离开仍须完成现有 flush。

封面限本地 PNG/JPEG/WebP、2 MB，以独立版本保存；项目备份可选地包含封面，旧备份不含封面也能恢复。schema 4→5 只新增书架和封面表，回填投影，不改旧正文、来源、版本与任务。Stage 4–6 的 World / Plot / 统一审查仍是原任务书范围，本阶段不借新导航宣称它们完成。
