...
| 场景 | 实际证据 | 状态 |
|---|---|---|
| 旧项目/schema 4 升级与恢复 | 只读 VACUUM INTO 两运行库，副本升级 schema 5，20 类旧表哈希/行数未变，integrity_check=ok；旧项目备份在副本独立恢复。正式 4318/4317 在无漂移后升级 v5，20 类旧表哈希不变，quick_check=ok，冻结旧项目 8/8 PASS。完整私有旧库保留。 | **PASS**；`scripts/stage3-{migrate-copy,runtime}.mjs`、`evidence/stage3-{migration,runtime}.json`、`evidence/upgrade-old-projects.json`、`test/stage3.test.ts` |
...
| Writer/Director 身份和定位 | 同一个项目从书架进入模式，URL/本地位置刷新恢复，离开清除项目 URL，不自动创建任务。旧会话门和编辑器 flush 保留；旧双模式测试继续通过。正式 DSH 页面首次检查发现有成果的旧项目把 artifact 摘要当任务摘要读取，修正并重测 3 个真实任务，无错误或写请求。 | **PASS（修复后）**；`npm test`、`evidence/stage3-{browser,runtime-browser}.json` |
...
当前 `npm test` 86 PASS、0 FAIL/skip，`npm run typecheck` / `npm run build` PASS；Stage 3 正式实例回归已完成。第三阶段未发起真实模型调用，其状态为 NOT_RUN；Stage 4–6 仍 NOT_RUN。下文 Stage 2 的“Stage 3–6 NOT_RUN”是历史状态，以本节为准；保留原始验收条件。
...
当前 `npm test` 83 PASS、0 FAIL/skip，`npm run typecheck`、`npm run build`、`git diff --check` PASS。Stage 3–6 **NOT_RUN**；原有 Stage 1 与 V1、原作及双模式验收记录不删除。下文 Stage 1 的“Stage 2–6 NOT_RUN”是其历史切片状态，以本节当前结果为准。
