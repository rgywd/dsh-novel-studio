...
迁移前先从运行中的 4318/4317 schema 3 库经**只读连接 VACUUM INTO**制作一致性副本，永久放在主工作区 `.local/backups/stage2-20260916/`（忽略、不入 Git）；只在副本执行 schema 4 迁移。13 类旧表行数及 SHA256 一致，两个副本 `integrity_check=ok` 且项目私人 JSON 备份都能独立恢复。随后主工作树快进合并并推送 `52a7fe0`，重建并按既有 PID 守护脚本重启 4318/4317：真实库增量升级到 v4，旧 13 类表哈希不变、quick_check=ok，旧项目回归 8/8 PASS，元数据/导航 revision 一致。见 `evidence/stage2-{migration,runtime}.json`。隔离服务 4322 使用全新测试库，实际浏览器创建项目、以显式演示提供方执行 Director、打开分页检查器并选中实际请求，详情有 P0/P1/P5 和 UNKNOWN 缓存计量；没有使用真实模型。Stage 3 尚未开始。

最近修改文件：`src/{read-model,projections,requests,store,scope,temporal,memory,context,domain,http,contracts}.ts`、`src/ui/{main,CreativeConfig,RequestInspector,SourceLibrary}.tsx`、`test/{stage2,configuration}.test.ts`、`scripts/stage2-{scale,migrate-copy,runtime}`、`package.json` 及本段文档/证据。当前未提交内容仅为 Stage 2 正式运行回执与恢复脚本，将单独提交并推送。Stage 2 工作树的 4322 隔离进程应在不再使用时核对 PID 停止；再清理该独立工作树。下一可执行任务是 Stage 3，保持 Writer/Director 共用同一本项目和现有访问控制。
