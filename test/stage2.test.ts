...
test('stage 2: schema 3 request rows migrate additively and old backup restores as independent copy',()=>{
  ...
  const old=new DatabaseSync(path);old.exec("DROP TABLE request_details; DROP TABLE request_summaries; DROP TABLE project_covers; DROP TABLE project_shelf; UPDATE meta SET value='3' WHERE key='schema';");const raw=String(old.prepare('SELECT data FROM requests WHERE id=?').get(legacy.id)!.data);old.close();
  const store=new Store(path),domain=new Domain(store);assert.equal(store.db.prepare("SELECT value FROM meta WHERE key='schema'").get()?.value,'5');assert.equal(String(store.db.prepare('SELECT data FROM requests WHERE id=?').get(legacy.id)!.data),raw);assert.deepEqual(store.objects(f.p.id),before);assert.equal(requestGet(store,f.p.id,legacy.id).rawResponse,legacy.rawResponse);const copy=domain.restoreBackup(domain.backup(f.p.id));assert.notEqual(copy.id,f.p.id);assert.equal(requestSummaries(store,copy.id).total,1);store.close();rmSync(dir,{recursive:true});
});
